...
class KDTree {
    ...
    KDNode* ins(KDNode* n, const VectorItem& v, int d) {
        ...
        int ax = d % dims;
        if (ax >= (int)v.emb.size() || ax >= (int)n->item.emb.size()) return n;
             ...
             std::priority_queue<std::pair<float,int>>& heap)
    {
        if (!n) return;
        int ax = d % dims;
        if (ax >= (int)q.size() || ax >= (int)n->item.emb.size()) return;
...
// Uses a boundary heuristic to skip false key matches inside string values
std::string extractStr(const std::string& body, const std::string& key) {
    std::string search = "\"" + key + "\"";
    size_t p = 0;
    while (true) {
        p = body.find(search, p);
        if (p == std::string::npos) return "";
        // Check that this key sits after { or , (skip keys found inside values)
        if (p > 0) {
            size_t bp = p - 1;
            while (bp > 0 && (body[bp] == ' ' || body[bp] == '\t' ||
                              body[bp] == '\n' || body[bp] == '\r')) bp--;
            char before = body[bp];
            if (before == '{' || before == ',' || bp == 0) break;
        } else {
            break;
        }
        p += search.size();
...
int main() {
    ...
    svr.Delete(R"(/delete/(\d+))", [&](const httplib::Request& req, httplib::Response& res) {
        ...
        catch (...) { res.set_content("{\"error\":\"invalid id\"}", "application/json"); return; }
    ...
    svr.Get("/doc/list", [&](const httplib::Request&, httplib::Response& res) {
        ...
        for (size_t i = 0; i < docs.size(); i++) {
            if (i) ss << ',';
            // Truncate text preview to 120 bytes, UTF-8 safe
            std::string preview;
            if (docs[i].text.size() <= 120) {
                preview = docs[i].text;
            } else {
                preview = docs[i].text.substr(0, 120);
                // Walk back if we cut in the middle of a UTF-8 multi-byte sequence
                while (!preview.empty() && ((unsigned char)preview.back() & 0xC0) == 0x80)
                    preview.pop_back();
                preview += "…";
            }
            ss << "{\"id\":" << docs[i].id
